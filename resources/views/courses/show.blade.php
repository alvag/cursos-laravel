                    ...
                    <ul class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                        @foreach($course->goals as $goal)
                            <li class="text-gray-700 text-base">
                                <i class="fas fa-check text-green-600 mr-4"></i> {{$goal->name}}
                            </li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="card-body">
                    ...
                    @can('enrolled', $course)
                        ...
                        <a href="{{route('courses.status', $course)}}"
                           class="btn btn-danger btn-block mt-4 focus:outline-none">Continuar con el curso</a>
                    @else

                        <form action="{{route('courses.enrolled', $course)}}" method="post">
                            @csrf
                            <button type="submit" class="btn btn-danger btn-block mt-4 focus:outline-none">Llevar este
                                curso
                            </button>
                ...
                @foreach($similares as $similar)
                    <article class="flex mb-6">
                        <img class="h-32 w-40 object-cover"
                             src="{{Storage::url($similar->image->url)}}" alt="{{$similar->title}}">
